...
function getChromiumPath() {
    ...
    return findFile(basePath, process.platform === 'win32' ? 'chrome.exe' : 'chrome');
}
...
ipcMain.handle('launch-profile', async (event, profileId) => {
    ...
    try {
        ...
        if (!chromePath) { await forceKill(xrayProcess.pid); throw new Error("Chrome binary not found."); }
        ...
        const browser = await puppeteer.launch({
            ...
            // 核心：不使用 ignoreDefaultArgs ['--enable-automation']
            // 因为它有时候会破坏 Extension 加载。如果你发现 Extension 不加载，尝试注释掉这行
            // 但为了 Stealth，通常建议保留，我们只移除插件依赖
